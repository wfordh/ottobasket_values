...
def main():
    ...
    for year in range(1996, 2023):
        ...
        # season_df = pd.DataFrame(response_json["rowSet"], columns=response_json["headers"])
        # season_df["season"] = params["Season"]
        season_df = pd.read_csv(f"./data/box_stats_{season}")
        season_df.drop(
            [col for col in season_df.columns if "RANK" in col], axis=1, inplace=True
        )
        fantasy_columns = [
            "pts",
            "reb",
        ...
        season_df = season_df.merge(
            player_idx_df[
                ["player_id", "position", "draft_year", "from_year", "to_year"]
            ],
            on="player_id",
            how="left",
        )
