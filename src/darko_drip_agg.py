...
def main():
    ...
    current_darko_df.to_csv("~/Downloads/darko.csv", index=False)
    current_drip_df.to_csv("~/Downloads/drip.csv", index=False)

    name_map = pd.read_csv("~/Downloads/drip_nba_id_mapping.csv")
    otto_nba = pd.read_csv("~/Downloads/otto_nba_id_merge.csv")
    ...
    draftable_players.extend(
        fantasy_pts_df.loc[
            (fantasy_pts_df.center_rk < 12) & (fantasy_pts_df.surplus_position == "C")
        ].nba_player_id.tolist()
    )
    draftable_players.extend(
        fantasy_pts_df.loc[
            (fantasy_pts_df.is_forward) & (~fantasy_pts_df.nba_player_id.isin(draftable_players))
        ].sort_values(by="proj_fantasy_pts_fs", ascending=False).nba_player_id.head(24).tolist()
    )
    draftable_players.extend(
        fantasy_pts_df.loc[
            (fantasy_pts_df.is_guard) & (~fantasy_pts_df.nba_player_id.isin(draftable_players))
        ].sort_values(by="proj_fantasy_pts_fs", ascending=False).nba_player_id.head(36).tolist()
    )
    draftable_players.extend(
        fantasy_pts_df.loc[
            ((fantasy_pts_df.is_forward) | (fantasy_pts_df.is_center)) & (~fantasy_pts_df.nba_player_id.isin(draftable_players))
        ].sort_values(by="proj_fantasy_pts_fs", ascending=False).nba_player_id.head(12).tolist()
    )
    draftable_players.extend(
        fantasy_pts_df.loc[
            ((fantasy_pts_df.is_forward) | (fantasy_pts_df.is_guard)) & (~fantasy_pts_df.nba_player_id.isin(draftable_players))
        ].sort_values(by="proj_fantasy_pts_fs", ascending=False).nba_player_id.head(12).tolist()
    )
    draftable_players.extend(
        fantasy_pts_df.loc[
    ...
    replacement_values = fantasy_pts_df.loc[
        ~fantasy_pts_df.nba_player_id.isin(draftable_players)
    ].groupby("surplus_position").proj_fantasy_pts_fs.max().to_dict()
    print(replacement_values)
    print(fantasy_pts_df.loc[
        fantasy_pts_df.nba_player_id.isin(draftable_players)
    ].player.tolist())
    ...
    fantasy_pts_df["surplus_value"] = fantasy_pts_df.proj_fantasy_pts_fs*surplus_factor + 1
    ...
    surplus_factor = (4800 - len(fantasy_pts_df.loc[fantasy_pts_df.points_above_repl > 0])) / total_league_value
    print(f"Total league value (take 2): {total_league_value}")
    print(f"Surplus factor (take 2): {surplus_factor}")
    fantasy_pts_df["surplus_value"] = fantasy_pts_df.points_above_repl*surplus_factor + 1

    fantasy_pts_df.to_csv("./ottoneu_fantasy_pts.csv", index=False)
